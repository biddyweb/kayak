...
import org.mapsforge.applications.android.samples.SeamarkImagesFromSvgActivity;
...
public class SeamarkSymbol {
    ...
    private static  InputStream mInputStreamFromAsset;
    ...
    private static boolean mSymbolsReadComplete = false;


    public static String getSVG_File_Name (String fileName) {
        String externalPathName = Environment.getExternalStorageDirectory().getAbsolutePath();
        String aPathName = externalPathName + "/" + SeamarkImagesFromSvgActivity.DEFAULT_APP_DATA_DIRECTORY+"/"+SEAMARK_SVG_DATA_DIRECTORY + "/" + fileName;
        return aPathName;
    }



    public static void loadDefsFromAssetsStream(Context pContext, String defsFileName){
        final String aFilename = defsFileName;
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        aInputStream =  context.getAssets().open(aFilename);
                        Reader reader = new InputStreamReader(aInputStream, "UTF-8");
                        XmlPullParserFactory parserCreator;
                        parserCreator = XmlPullParserFactory.newInstance();
                        XmlPullParser parser = parserCreator.newPullParser();

                        parser.setInput(reader);
                         //XmlPullParser parser = assetManager.openXmlResourceParser(aFilename);
                                ...
                                case XmlPullParser.END_TAG: {
                                    ...
                                    if (tag.compareTo("symbol") == 0) {
                                        if (!mSymbolsDictionary.containsKey(symbolName)){
                                            ...
                                            //Log.d(TAG,"Scanning defs file found:  " + symbolName);
                                        }
                                    }
    ...
    public static boolean getSymbolDefsComplete() {
        return mSymbolsReadComplete ;
    }
